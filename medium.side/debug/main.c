...
void ShipVertical(int ship[COL][ROW],int,int,int);
void ShipHorizontal(int ship[COL][ROW],int,int,int);
void Step1_4(int ship[COL][ROW],int*,int*,int*,int*,int*,int*);
void Step5_6(int ship[COL][ROW],int*,int*,int*,int*);
void Step7_10(int ship[COL][ROW]);
void Debuger(int ship[COL][ROW],int*,int*,int*,int*);
void Counter(int ship[COL][ROW],int*,int*,int*,int*);
int* NumGen(int*);
...
void Step1_4(int ship[COL][ROW],int *num, int *shN, int *side1, int *side2, int *side3, int *side4)
{
	int side[4]={1,2,3,4};
	int i,tmp,swap;
	srand(time(NULL));
	...
	for(i=0;i<4;i++)
	{
		if (*(side+i) == 1 || *(side+i) == 3)
		{
			if (*(side+i)==1)
			{
				...
				printf("side=%d, 1=%d, 10=%d, num=%d, shN=%d\n",side[i],side1[0],side1[1],num[i],shN[i]);
			}
			if (*(side+i)==3)
			{
				...
				printf("side=%d, 1=%d, 10=%d, num=%d, shN=%d\n",side[i],side3[0],side3[1],num[i],shN[i]);
			}
		} //if (*(side+i) == 1 || *(side+i) == 3)
		else
		{
			if (*(side+i)==2)
			{
				...
				printf("side=%d, 1=%d, 10=%d, num=%d, shN=%d\n",side[i],side2[0],side2[1],num[i],shN[i]);
			}
			if (*(side+i)==4)
			{
				...
				printf("side=%d, 1=%d, 10=%d, num=%d, shN=%d\n",side[i],side4[0],side4[1],num[i],shN[i]);
			}
		} //else ------ if (*(side+i) == 1 || *(side+i) == 3) 
	} //for(i=0;i<4;i++)	
}//void Step1_4();

void Step5_6(int ship[COL][ROW], int *side1, int *side2, int *side3, int *side4)
{
	int s1,s2,s3,s4, num;
	s1 = side1[1]-side1[0];
	s2 = side2[1]-side2[0];
...
void Step7_10(int ship[COL][ROW])
{
	...
	for(i=0;i<4;i++)
	{
		...
		ship[rw+1][cl+1]=1;
		ship[rw+1][cl]=1;
		printf("1-%d: rw=%d, cl=%d\n",i,rw,cl);
	}
}//void Step7_10

void ShipVertical(int ship[COL][ROW], int shN, int rw, int cl)
{
	int i;
	for(i=0;i<shN;i++)
	{
...
void ShipHorizontal(int ship[COL][ROW], int shN, int rw, int cl)
{
	int i;
	for(i=0;i<shN;i++)
	{
...
void Debuger(int ship[COL][ROW],int *side1, int *side2, int *side3, int *side4)
{
...
void Counter(int ship[COL][ROW], int *side1, int *side2, int *side3, int *side4)
{
	int i=0,ctr=0,n=0;
	...
	printf("side1: {%d,%d} {%d,%d} {%d,%d}\n", side1[0], side1[1], side1[2], side1[3], side1[4], side1[5]);
	printf("side2: {%d,%d} {%d,%d} {%d,%d}\n", side2[0], side2[1], side2[2], side2[3], side2[4], side2[5]);
	printf("side3: {%d,%d} {%d,%d} {%d,%d}\n", side3[0], side3[1], side3[2], side3[3], side3[4], side3[5]);
	printf("side4: {%d,%d} {%d,%d} {%d,%d}\n", side4[0], side4[1], side4[2], side4[3], side4[4], side4[5]);
